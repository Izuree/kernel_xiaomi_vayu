...
static int __init ir_spi1_init(void)
{
	/* Abort silently if the boot parameter wasn't passed */
	if (!e404_data.ir) {
		pr_info("E404: prevent ir1 from loading");
		return 0;
	}
...
static void __exit ir_spi1_exit(void)
{
	if (e404_data.ir)
		spi_unregister_driver(&ir_spi_driver);
}

module_init(ir_spi1_init);
module_exit(ir_spi1_exit);
